...
def data_js():
    try:
        ...
        # Per-trip pidCatalog is identical across trips and redundant with the
        # global PID_CATALOG below — strip it to keep the payload small.
        slim_trips = _safe([{k: v for k, v in t.items() if k != "pidCatalog"} for t in trips])
