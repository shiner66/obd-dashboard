...
def _to_local_iso(date_str: str) -> str:
    """Strip the spurious Z suffix from Stellantis timestamps.

    Stellantis sends Italian local time with a Z suffix (wrong label).
    The raw value is already Italian local time for most trips; the correlator
    scoring handles the occasional trip where Stellantis applied DST twice
    (group B) by trying both raw and DST-adjusted time in its score function.
    """
